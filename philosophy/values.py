...
def format_values_section(raw: dict[str, float]) -> str:
    ...
    if not has_signal(raw):
        return ""
    L = ["\n## 가치 프로파일 (Schwartz 기본가치)"]
    tops = top_values(raw)
    if tops:
        L.append("지향: " + " · ".join(f"{n}({q}) +{s}" for n, q, s in tops))
    demo = demoted_values(raw)
    if demo:
        L.append("배척: " + " · ".join(f"{n} {s}" for n, s in demo))
    return "\n".join(L)
