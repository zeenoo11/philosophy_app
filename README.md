...
philosophy/        철학 탐구 서비스 — PhiloGraph graphRAG (Graph-Project C_RAG 이식)
  graph.py         unified_graph.json 로더 + asserts/opposes/value/인접 인덱스 + BFS 확장
  embed.py         fastembed(all-MiniLM-L6-v2 ONNX) — 노드 임베딩 사전계산 + 쿼리 임베딩
  retriever.py     텍스트 cosine 회수 + 유사 주장의 실제 저자(asserts) 랭킹
  values.py        Schwartz 가치 프로파일 — 가치층 결정론 집계 + 8각 축 변환
  decompose.py     LLM 명제 분해 + 영어 정규화 (규칙 폴백)
  diagnose.py      Diagnosis 조립·포맷 (+ 가치 프로파일 섹션)
...
가치관 한 문장 → decompose (LLM: 핵심 명제 2~4개 + 영어 정규화)
             ...
             → values    (Schwartz 가치층: claim→value promotes/demotes × 유사도 집계)
             → diagnose  (유사 주장·가까운 철학자·대비 입장·가치 프로파일 + 원문 인용)
             → LLM 진단문 (OpenRouter 스트리밍, 근거 안에서만 서술)
```

**Schwartz 가치 프로파일** — Graph-Project Plan 3(`PLAN_schwartz_value_layer.md`)이
구체화한 가치층(value 노드 10종, promotes/demotes 엣지 597건·weight)을 그대로 사용해
사용자 프로파일(10차원)을 결정론으로 집계한다. `/me` 에서 circumplex 순서의
**8각 레이더**(인접 가치 병합: 자극·쾌락 / 전통·동조)로 그려지며, 마주보는 축이
이론적 긴장(자기주도↔안전, 성취↔자애)과 일치한다. 가치 표명이 없는 입력은
빈 프로파일이 정상(원 계획의 abstain 정신).
